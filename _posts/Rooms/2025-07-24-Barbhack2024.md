...
```
Something worth noting—aside from the `READ, WRITE` permissions on a share in `SRV01`—is that both `SRV01` and `SRV02` have `SMB signing: False`, which *might* be useful later for relaying.
...
The logic of the code is quite easy, just checking if `input_key` matches the key that is in `C:\SHARE\key.txt`, or if the `input_key` equals 24 chars includes the word `GOTHAMCITY`. And we can see it actually tries to open it in `procmon`
...
```
but it didn't work somehow, remeber this code is decompiled so the flow of the code maybe different from the actual one.

### 1st Flag
after creating the desired `key.txt` we got back the 1st flag!
![image](/assets/img/CTFs/Gotham_city/1st.png)

> I tried `aaaaaaaaaaaaaaGOTHAMCITY` string again to stasfiy my curiosity, will it work know with the file in the correct path?
I tried to input that 24 char string and it worked, but it still reads the 5 char `pix3l` from the `key.txt`.
